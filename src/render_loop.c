...
{
	static t_xpm	*xpm;if (!xpm)	xpm = ft_xpm_image("/home/pedromar/code/minirt_/files_xpm/spacee.xpm");
	t_vec4	color;
	int		i;
...
int	render_loop(t_render *r)
{
	t_ivec2	pixel;
	t_hit	h;
	static t_xpm	*xpm; if (!xpm) 	xpm = ft_xpm_image("/home/pedromar/code/minirt_/files_xpm/sky.xpm");
INIT_CLOCK
	pixel.y = -1;
	while (++pixel.y < WIN1_SY)
	{
		...
		while (++pixel.x < WIN1_SX)
		{
			...
			{
				put_pixel(r->canvas, &pixel, rgba_to_int(get_pixel_xpm(xpm, &(t_vec2){.x = 1.3f * (0.5f + atan2f(h.primary.d.z, h.primary.d.x) / (2.0f * M_PI)), .y = 1.3f * (0.5f - asinf( h.primary.d.y) / M_PI)})));
				continue ;
			}
		}
	}
